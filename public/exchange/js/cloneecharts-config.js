    ...
    switch (message.action) {
      ...
      case 'main':
        ...
        if (timeSV == 0) {
          ...
          $('.btn-buy span').html(buyLang);
          $('.btn-sell span').html(sellLange);
...
function getHistory(_t = 'CLOSE') {
    $.get(base_url + "/history", { status: _t }, function (data) {
        ...
        data.forEach(function (item) {
          	_html += '<li role="presentation" class="account-picture history-trades mt-2"><div class=" cursor-pointer grid grid-cols-5 px-3" ><div class="col-span-3 flex flex-col flex-1"><span class="balance-value">'+amountLange+': $' + Number(item.GameBet_Amount).toFixed(2) + '</span><span class="balance-value">'+profitLange+': $' + Number(item.GameBet_AmountWin).toFixed(2) + '</span><span class="balance-name">'+timeLange+': ' + timeConverterv2(item.GameBet_datetime) + '</span></div><div class="col-span-2 flex flex-col justify-center self-center items-center"><img src="' + base_url + '/exchange/img/icon/BTC.png" alt=""><span class=" white balance-name text-center">BTCUSDT ' + ((item.GameBet_Currency == 99) ? '(demo)' : '(Live)') + ' </span></div></div></li>';
        });
...
function convertStatistical(_d) {
  ...
  _htmlStatistical += '</div>';
	...
	$('.up-water').html( _upWater + buyLang);
	$('.down-water').html( _downWater + sellLange);
...
function convertOrder(_d) {
  ...
  _html = '<tr class="order-' + _d.order + '"><td>' + _d.user + '</td><td>' + _d.symbol + '</td><td class="uppercase ">' + _d.order + '</td><td>' + ((_d.amount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')) + '</td><td>' + convertTimetoDate(_d.time * 1000) + '</td></tr>';

  if(w < 1280){
               _fullHtml = $('#order-mb tbody').html();
               _fullHtml = _html + _fullHtml;
               $('#order-mb tbody').html(_fullHtml);

  }
  else{
               _fullHtml = $('#order-ds tbody').html();
               _fullHtml = _html + _fullHtml;
               $('#order-ds tbody').html(_fullHtml);
  }
